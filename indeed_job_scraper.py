...
def get_job_link(job_search_url):
    ...
    Given the url for specific job search, this function returns the
     url for all the open position for that job
    ...
    try:
        ...
        if number_of_pages > 1:   # if there are more than one pages of job advertising
            start = 10
            # we start from page  2 as for page 1 we have already dealt above
            for i in range(2, number_of_pages+1): # we start from page
        ...
        print("Something wrong with URL. Please check Your input and try again.")
        sys.exit()
...
def count_words(job_urls):
    """
    Given the list of urls to all open position, this function returns the number of occurance of
    key job skill (skill count) in those vacancies.
    """
    word_counter = Counter()  # declaring an empty counter
    for url in job_urls:
    ...
    skill_to_count = []
    skill_to_count = read_file() ## call the function to read the file containing words to count
    skill_counter = {}
    if skill_to_count:
        for word in skill_to_count:
           skill_counter[word] = word_counter[word]
...
def generate_url():
    """
        this function takes the job title and location as user input and
        return the main indeed job url.
...
def read_file():
    """
      This function reads the text file that contains job skills , whose occurance
      will be counted in job vacancies.
    """
    words_to_count = []
    try:
        with open('words_to_counts.txt', 'r') as f:
            for line in f:
                line = line.strip().split(',')
                for word in line:
                    if word == "":
                        continue
                    words_to_count.append(word.strip().lower())
        return words_to_count
    except Exception:
        print("The file containing the job skill was not found.")
        print("Programe will return default skill counts.")
        print('\n')
